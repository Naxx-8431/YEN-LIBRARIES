    ...
    <div class="admin-main">
      <header class="topbar">
        <div class="topbar__left"><button class="topbar__burger" onclick="toggleSidebar()"><svg
          ...
          <h1 class="topbar__title">Services Manager</h1>
        </div>
      ...
      <div class="admin-content">
        <div class="page-header">
          <div class="page-header__left">
            ...
            <p>Manage library services displayed on the website</p>
          </div>
        </div>
        ...
        <div class="card mb-3">
          <div class="card__header">
            <div class="card__title"><?php echo $edit ? '✏️ Edit Service' : '➕ Add New Service'; ?></div>
          </div>
          <div class="card__body">
            <form method="POST" action="">
              <?php if ($edit): ?><input type="hidden" name="service_id"
                  value="<?php echo $edit['id']; ?>"><?php endif; ?>
              <div class="form-grid">
                <div class="form-group"><label class="form-label">Service Title *</label><input type="text" name="title"
                    class="form-input" required value="<?php echo $edit ? htmlspecialchars($edit['title']) : ''; ?>"
                    placeholder="e.g. OPAC / Web-OPAC"></div>
                <div class="form-group"><label class="form-label">Section</label><input type="text" name="section"
                    class="form-input" value="<?php echo $edit ? htmlspecialchars($edit['section']) : ''; ?>"
                    placeholder="e.g. Search & Discovery"></div>
                <div class="form-group"><label class="form-label">Link URL</label><input type="url" name="link_url"
                    class="form-input" value="<?php echo $edit ? htmlspecialchars($edit['link_url']) : ''; ?>"
                    placeholder="https://..."></div>
                <div class="form-group"><label class="form-label">Link Label</label><input type="text" name="link_label"
                    class="form-input" value="<?php echo $edit ? htmlspecialchars($edit['link_label']) : ''; ?>"
                    placeholder="e.g. Access OPAC"></div>
              </div>
              <div class="form-group"><label class="form-label">Description</label><textarea name="description"
                  class="form-input" rows="3"
                  placeholder="Describe the service..."><?php echo $edit ? htmlspecialchars($edit['description']) : ''; ?></textarea>
              </div>
              <?php if ($edit): ?>
        ...
        <div class="card">
          <div class="card__header">
            <div class="card__title">All Services (<?php echo mysqli_num_rows($services); ?>)</div>
          </div>
              ...
              <tbody>
                <?php while ($s = mysqli_fetch_assoc($services)): ?>
                  <tr>
                    ...
                    <td>
                      <?php echo $s['link_url'] ? '<a href="' . htmlspecialchars($s['link_url']) . '" target="_blank" style="font-size:12px;">' . htmlspecialchars($s['link_label'] ?? 'Link') . '</a>' : '—'; ?>
                    </td>
                    <td><a href="services-manager.php?toggle=<?php echo $s['id']; ?>"
                        style="font-size:20px;"><?php echo $s['visible'] ? '🟢' : '⚪'; ?></a></td>
                    <td>
                      <a href="services-manager.php?edit=<?php echo $s['id']; ?>" class="btn btn--ghost btn--sm">✏️</a>
                      <a href="services-manager.php?delete=<?php echo $s['id']; ?>" class="btn btn--ghost btn--sm"
