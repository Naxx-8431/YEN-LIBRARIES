...
if ($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['add_library'])) {
    ...
    if (empty($name) || empty($slug)) {
        ...
        $slug_check = mysqli_query($conn, "SELECT id FROM libraries WHERE slug='$slug' LIMIT 1");
        if (mysqli_num_rows($slug_check) > 0) {
            $message = "A library with the slug '$slug' already exists. Please choose a different slug."; $msg_type = "error";
        } else {
...
if ($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['edit_library'])) {
    ...
    // Check for duplicate slug before updating (exclude current library)
    $slug_check = mysqli_query($conn, "SELECT id FROM libraries WHERE slug='$slug' AND id!=$id LIMIT 1");
    if (mysqli_num_rows($slug_check) > 0) {
        $message = "A library with the slug '$slug' already exists. Please choose a different slug."; $msg_type = "error";
    } else {
